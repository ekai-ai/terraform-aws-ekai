variable "ekai_namespace" {
  description = "Kubernetes namespace for ekai services"
  type        = string
}
  ...
  description = <<-EOT
    ArgoCD Application source:
      ...
      "helm" — self-service clients. No git/CI access needed at all — deploys a
               ...
               helm_chart_name/helm_chart_version). Chart itself is not built
               yet — this only wires the option.
  EOT
  type        = string
  default     = "git"
  validation {
...
variable "manifest_folder" {
  description = "Folder inside deployment-files repo where K8s manifests live (source_type = \"git\" only)"
  type        = string
...
variable "helm_values" {
  description = "Raw YAML values passed to the chart (source_type = \"helm\" only) — e.g. image tags, hostnames. Schema depends on the chart, which does not exist yet."
  type        = string
  default     = ""
}
