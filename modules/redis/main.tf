...
# Connection details flow to 03-platform outputs and are merged into per-service
# Secrets Manager secrets by the 04-cicd layer.
locals {
  redis_host = "redis-master.${var.redis_namespace}.svc.cluster.local"
  redis_port = 6379
  # "<host>:<port>", no scheme, no embedded credentials -- matches
  # REDIS_URI as actually used in a real, verified-working Ekai environment
  # (GCP Knowledge's secret: "redis-stack-master.redis.svc.cluster.local:6379").
  # Not "redis://user:pass@host:port" (that key/format doesn't appear in any
  # real Ekai secret checked -- AWS dev, GCP Knowledge -- and caused the
  # backend to hang indefinitely retrying "Getting Redis client" every ~5s
  # with no error surfaced.
  redis_uri = "${local.redis_host}:${local.redis_port}"
}
