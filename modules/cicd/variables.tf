...
# Wired automatically from module.cluster's outputs by the root module (see
# ../../main.tf) — nothing to set in tfvars for these.

variable "aws_account_id" {
  description = "AWS account ID (sourced from the cluster submodule's output)"
  type        = string
}

variable "backend_db_username" {
  description = "Backend RDS DB username (sourced from the cluster submodule's output, self-service only)"
  type        = string
  sensitive   = true
  default     = ""
}
...
variable "backend_db_password" {
  description = "Backend RDS DB password (sourced from the cluster submodule's output, self-service only)"
  type        = string
  sensitive   = true
  default     = ""
}
...
variable "backend_db_name" {
  description = "Backend RDS DB name (sourced from the cluster submodule's output, self-service only)"
  type        = string
  default     = ""
}
...
variable "private_subnet_ids" {
  description = "Private subnet IDs (sourced from the cluster submodule's output)"
  type        = list(string)
}

variable "rds_sg_id" {
  description = "RDS security group ID (sourced from the cluster submodule's output)"
  type        = string
}
...
# Wired automatically from module.platform's outputs by the root module (see
# ../../main.tf) — nothing to set in tfvars for these.
...
variable "redis_credentials" {
  description = "Redis connection details (sourced from the platform submodule's output). Empty map when enable_redis = false."
  type        = map(string)
  sensitive   = true
  default     = {}
}
...
# Wired automatically from module.bootstrap's outputs by the root module (see
# ../../main.tf) — nothing to set in tfvars for these.
