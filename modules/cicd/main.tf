...
# just boot. Only values that genuinely require the client's own external
# accounts — ones no amount of Terraform can substitute for (LLM API keys,
# Cognito, an IAM user with S3/SES access, GCP Document AI) — are left as
# "REPLACE_ME". Update those after apply with (see the
# `client_iam_policy_for_secret_placeholders` output for the IAM user's
# S3/SES policy, and the ekai-saas chart README "Setting up the application
# secret" for what each placeholder key is for):
...
locals {
  ...
  # database, VECTOR_DATABASE_URL for semantics' (same RDS instance, separate
  # database + user).
  ...
  generated_db_urls = local.self_service ? {
    ...
    VECTOR_DATABASE_URL = "postgresql://${var.semantics_db_username}:${var.semantics_db_password}@${var.rds_endpoint}/${var.semantics_db_name}"
  } : {}
  ...
  # server-to-server -- same mistake as the FRONTEND__ URLs below, just one
  # layer deeper: pointing these at the public Ingress hostname sends the
  # backend's own traffic out through the internet-facing ALB and back in
  # (AWS ALB hairpin routing), which surfaced as intermittent 503s
  # ("Profile status request failed") even though the same hostname works
  # fine from an external browser. local-deployment/.env.example already
  # gets this right for AI_CORE_ENDPOINT (http://ekai-erd:9002, a Docker
  # Compose network alias) -- the K8s equivalent is the in-cluster Service
  # DNS name. Ports match the chart's values.yaml containerPort defaults
  # (backend:3000, semantics:8000, profile:9002, erd.api:9002); this layer
  # has no visibility into those defaults today, so they're hardcoded here
  # same as local-deployment's .env.example hardcodes them.
  #
  ...
  # (/api/, /api/ai_core_ms/, /api/ai_core_semantics/, /api/ai_core_profiler/
  # -> the same backend/erd/semantics/profile K8s Services). Absolute
...
# erd/semantics/profile.<dns_zone>) would be NXDOMAIN after a successful
# apply — nothing else in this stack creates these records.
